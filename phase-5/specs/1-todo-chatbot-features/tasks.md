...
- [x] T008 [P] Configure Dapr Pub/Sub component for Kafka in `deploy/dapr-components/kafka-pubsub.yaml`.
- [x] T009 [P] Configure Dapr State Store component for PostgreSQL in `deploy/dapr-components/postgres-statestore.yaml`.
- [x] T010 [P] Configure Dapr Secrets component (if needed for API keys/credentials) in `deploy/dapr-components/secrets.yaml`.
- [x] T011 [P] Setup error handling and logging infrastructure for microservices in `src/shared/`.
- [x] T012 [P] Setup common Dapr client and service invocation utility in `src/shared/dapr_utils.py`.
...
- [x] T033 [P] [US5] Update Task model (add `due_date` DateTime, `reminder_settings` JSON Object) in `src/shared/models.py`.
- [x] T034 [P] [US5] Create Notification Service microservice scaffolding in `src/notification_service/`.
- [x] T035 [US5] Implement API endpoint for creating/managing notifications in `src/notification_service/api/notification_api.py`.
- [x] T036 [US5] Notification Service: subscribe to `reminder_triggered` event from `reminders` topic in `src/notification_service/events/reminder_events.py`.
- [x] T037 [US5] Implement API endpoint for setting task due date/time/reminder in `src/task_service/api/task_api.py`.
- [x] T038 [US5] Task Service: publish `reminder_triggered` event to `reminders` topic based on `reminder_settings` in `src/task_service/events/task_events.py`.
- [x] T039 [US5] Integrate chatbot command processing for "set due date", "remind me" in `src/api_gateway/nl_processor.py`.
...
- [x] T040 [P] [US6] Refine event schemas for `task_created`, `task_updated`, `reminder_triggered`, `recurring_task_instance_created` in `src/shared/events.py`.
- [x] T041 [US6] Implement Dapr Pub/Sub event publishers in `src/task_service/events/`, `src/recurring_task_service/events/`.
- [x] T042 [US6] Implement Dapr Pub/Sub event subscribers in `src/notification_service/events/`, `src/recurring_task_service/events/`.
- [x] T043 [US6] Configure Dapr Pub/Sub bindings in `deploy/dapr-components/kafka-pubsub.yaml` to ensure topics (e.g., `task-events`, `reminders`, `task-updates`) are correctly mapped.
...
- [x] T044 [P] [US7] Extend Helm chart for `api_gateway` microservice in `deploy/helm/todo-chatbot-features/charts/api-gateway/`.
- [x] T045 [P] [US7] Extend Helm chart for `task_service` microservice in `deploy/helm/todo-chatbot-features/charts/task-service/`.
- [x] T046 [P] [US7] Create Helm chart for `notification_service` microservice in `deploy/helm/todo-chatbot-features/charts/notification-service/`.
- [x] T047 [P] [US7] Create Helm chart for `recurring_task_service` microservice in `deploy/helm/todo-chatbot-features/charts/recurring-task-service/`.
- [x] T048 [US7] Update main Helm chart `deploy/helm/todo-chatbot-features/Chart.yaml` and `values.yaml` to include new microservices and Dapr annotations.
- [x] T049 [US7] Deploy all Dapr components and microservices to Minikube using the Helm chart, verify successful deployment.
