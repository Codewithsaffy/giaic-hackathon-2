...
The system needs to process task-related actions (creation, updates, reminders) as events flowing through Redis, handled by dedicated microservices.
...
**Independent Test**: Can be fully tested by performing a task action (e.g., creating a task) and verifying that the corresponding event is published to, and consumed from, the appropriate Redis topic by the relevant microservice.

**Acceptance Scenarios**:

1.  **Given** I create a new task via the chatbot, **When** the task is saved, **Then** a `task_created` event is published to the `task-events` Redis topic.
...
**Independent Test**: Can be fully tested by deploying the manifests to Minikube, verifying all pods are running and Dapr sidecars are injected, and then performing end-to-end functional tests on the Minikube deployment.

**Acceptance Scenarios**:

1.  **Given** the manifests are applied to Minikube, **When** all services and Dapr components are deployed, **Then** all pods are in a 'Running' state, and Dapr sidecars are present on Dapr-enabled microservice pods.
...
-   What happens if Redis is down when a task event needs to be published?
...
-   **FR-001**: System MUST allow users to set task priorities as 1, 2, or 3.
-   **FR-002**: System MUST allow users to attach custom tags to tasks.
-   **FR-003**: System MUST provide search functionality for tasks based on keywords, priority, and tags.
-   **FR-004**: System MUST allow filtering of tasks by priority and tags.
-   **FR-005**: System MUST support sorting of tasks by creation date, due date, and priority.
-   **FR-006**: System MUST enable creation and management of recurring tasks with configurable schedules (e.g., daily, weekly).
-   **FR-007**: System MUST allow users to specify a due date and time for any task.
-   **FR-008**: System MUST send notifications to users for task reminders at a configured time.
-   **FR-009**: System MUST support natural language commands from the chatbot for all task management operations.
-   **FR-010**: System MUST publish task-related events (e.g., `task_created`, `task_updated`) to Redis topics.
-   **FR-011**: System MUST consume task-related events from Redis topics to trigger appropriate microservice actions.
-   **FR-012**: System MUST include a dedicated Notification Service responsible for sending user notifications.
-   **FR-013**: System MUST include a dedicated Recurring Task Service responsible for managing recurring task schedules.
-   **FR-014**: System MUST include a dedicated Audit Service for logging all system events.
-   **FR-015**: System MUST use Dapr for Pub/Sub functionality for event handling using Redis.
-   **FR-016**: System MUST use Dapr State Management for persistent storage of microservice state using Redis.
-   **FR-017**: System MUST leverage Dapr Jobs API for scheduling exact-time reminders.
-   **FR-018**: System MUST integrate Dapr Secrets for secure management of sensitive configuration.
-   **FR-019**: System MUST be deployable and verifiable on Minikube.
...
-   **Task**: Represents a user's task. Attributes include:
    ...
    -   `due_date`: (New) Optional datetime.
    -   `remind_at`: (New) Optional datetime for reminder.
    -   `recurring_interval`: (New) Optional string defining repetition (e.g., "daily", "weekly").
    -   `completed`: (Existing) Boolean.
    -   `user_id`: (Existing) String.
...
-   **SC-001**: All Intermediate features are fully functional via chatbot commands.
-   **SC-002**: All Advanced features (recurring tasks, reminders) are fully functional.
-   **SC-003**: 100% of task events are successfully published to and consumed from Redis topics.
-   **SC-004**: Dapr sidecars are successfully injected into all microservices.
-   **SC-005**: The system successfully deploys on Minikube with all pods 'Running'.
-   **SC-006**: Natural language commands are processed correctly by the chatbot.
