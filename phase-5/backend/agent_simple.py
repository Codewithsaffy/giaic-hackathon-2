...
# Initialize Cerebras client with tuned performance settings
client = AsyncOpenAI(
    api_key=api_key,
...
async def run_task_agent(user_id: str, message: str, mcp_server=None) -> str:
    ...
    try:
        ...
        if mcp_server:
            # Reusing persistent session (Fastest)
            return await execute_with_mcp(mcp_server)
        else:
            # Fallback to one-off connection (Slow - only for local testing without main.py)
            logger.warning("Agent fallback: Starting one-off MCP connection...")
